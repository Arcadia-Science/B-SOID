...
def get_env_variable(var_name, default_value=None):
    ...
    if default_value is None and not value:
        raise ValueError(
            f"Environment variable '{var_name}' is required and cannot be empty."
        )
    return value
...
try:
    ...
    exit(
        1
    )  # Exit if any required variable is missing or if a conversion to float fails
...
extractor = extract_features.Extract(
    WORKING_DIR, PREFIX, processed_input_data, FRAMERATE, FRACTION
)
extractor.main()

[_, sampled_embeddings] = load_embeddings(WORKING_DIR, PREFIX)
clusterer = clustering.Cluster(
    WORKING_DIR,
    PREFIX,
    sampled_embeddings,
    AUTOSAVE,
...
exporter = export_training.Export(
    WORKING_DIR, PREFIX, sampled_features, assignments, assign_prob, soft_assignments
)
exporter.save_csv()
...
learning_protocol = machine_learner.Protocol(
    WORKING_DIR, PREFIX, features, sampled_features, assignments
)
